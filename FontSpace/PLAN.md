...
- [x] Text preview — editable sample text painted with the page's glyphs (entry-gated) ([spec/12](spec/12-gui.md) §12.10, [spec/09](spec/09-rendering.md)). — [#21](https://github.com/enthal/glyph-80/pull/21) *(colors/inversion/zoom/spacing/wrap/presets/multi-page are follow-ups)*
- [x] Open/Save/Save As/Revert with atomic writes; dirty tracking. Shared atomic file I/O (`fontspace-json::{read_document, write_document}`, used by CLI + GUI), a File menu with native `rfd` dialogs, dirty tracking, and an unsaved-changes guard ([spec/12](spec/12-gui.md) §12.12, [spec/16](spec/16-performance-safety-limits.md) §16.2). — [#22](https://github.com/enthal/glyph-80/pull/22)
- [x] **Platform identity + Linux desktop integration** ([spec/18](spec/18-platform-support.md)): `APP_ID`/storage-namespace constants, embedded `assets/app_icon.png` → window icon, `with_app_id`, the self-installing `.desktop`+icon (Exec-resolves / AppImage / `StartupWMClass` / `update-desktop-database`), and the `cursor_env` re-exec bridge, with pure `desktop_entry_contents`/`desktop_exec_field`/`resolve_exec_path`/`cursor_env_additions` helpers unit-tested on every platform, plus the `APP_ID`-pin test. — identity + icon [#23](https://github.com/enthal/glyph-80/pull/23), Linux desktop entry + cursor bridge [#24](https://github.com/enthal/glyph-80/pull/24)
- [x] `egui_kittest` snapshots for editor, page overview, preview; snapshot review. *(Infrastructure landed in [#14](https://github.com/enthal/glyph-80/pull/14); each view slice shipped its own reviewed Linux/lavapipe baseline — glyph editor + after-stroke [#16](https://github.com/enthal/glyph-80/pull/16)/[#17](https://github.com/enthal/glyph-80/pull/17), page overview [#18](https://github.com/enthal/glyph-80/pull/18), character set [#19](https://github.com/enthal/glyph-80/pull/19), text preview [#21](https://github.com/enthal/glyph-80/pull/21), shell + dirty-titlebar + discard-modal [#22](https://github.com/enthal/glyph-80/pull/22); placeholder panes are covered by the shell snapshot.)*
...
- [ ] Fragments + `extract`/`paste` ops with explicit `GlyphMapping`/`PageMapping`/`GlyphSizeConversion` ([spec/08](spec/08-fragments-and-clipboard.md)). _In progress:_ glyph fragments (`FontSpaceFragment`/`GlyphFragment`) with `extract_glyphs`/`paste_glyphs`, `GlyphMapping::{ByCode, SequentialFromCode}`, and `GlyphSizeConversion::RequireExact` ([#31](https://github.com/enthal/glyph-80/pull/31)). Canonical fragment JSON (`save_fragment`/`load_fragment` + atomic file I/O — spec/08 §8.5) ([#32](https://github.com/enthal/glyph-80/pull/32)); CLI `extract` (single-page glyph fragment → fragment file) ([#33](https://github.com/enthal/glyph-80/pull/33)); CLI `paste` (fragment → page under `by-code`/`sequential-from-code`, `RequireExact`) ([#34](https://github.com/enthal/glyph-80/pull/34)); size conversions `PlaceAt`/`Center` (a `placed` bitmap primitive + paste `--size`) ([#35](https://github.com/enthal/glyph-80/pull/35)); `ScaleNearest` resampling conversion (a `scaled_nearest` primitive) ([#36](https://github.com/enthal/glyph-80/pull/36)); egui glyph copy/paste via the system clipboard ([#37](https://github.com/enthal/glyph-80/pull/37)); `BySlot` ordinal mapping ([#38](https://github.com/enthal/glyph-80/pull/38)). Still to come: `Crop`, `PageMapping`, and the `Pages`/`Objects`/`ExportComponents` fragment variants.
- [ ] Clipboard integration (custom fragment MIME + `text/plain` + `image/png`); cross-instance paste. _In progress:_ glyph editor `Cmd/Ctrl+C`/`V` copy/paste via the system clipboard, carrying canonical fragment JSON as `text/plain` (already cross-instance + CLI-compatible) ([#37](https://github.com/enthal/glyph-80/pull/37)); page-overview **drag-select a run of codes + multi-glyph copy** as one fragment ([#48](https://github.com/enthal/glyph-80/pull/48)). Still to come: the dedicated fragment MIME, human-readable `text/plain`, `image/png` preview, and copy/paste from the remaining multi-glyph views (text preview, character-across-pages).
...
- [ ] Page-overview selection operations (shift/clear/invert on a range); range copy/paste. _Landed early alongside the M3 clipboard work:_ drag-select a run of codes + multi-glyph copy ([#48](https://github.com/enthal/glyph-80/pull/48)); Blank/Invert on the run ([#49](https://github.com/enthal/glyph-80/pull/49)); Shift the run ([#50](https://github.com/enthal/glyph-80/pull/50)); `ByCode` paste-onto-page via an in-app fragment clipboard ([#51](https://github.com/enthal/glyph-80/pull/51)). Still to come: auto/fixed thumbnail sizing and label modes.
...
- [x] **Code-addressed** address/data maps; `evaluate_output_word`; logical memory-image generation — the `fontspace-export` crate with infallible `evaluate_output_word` + `generate_image` ([#54](https://github.com/enthal/glyph-80/pull/54)).
- [x] **1:1 coverage validator** (partition check, no address/data overlap, width match) with the diagnostics in [spec/14](spec/14-validation-and-errors.md), plus a byte-exact row-scan test and the AT28C64-shape summary ([#54](https://github.com/enthal/glyph-80/pull/54)). *(A `proptest` "validated config never reads OOB" is a follow-up; evaluation is bounds-safe by construction — checked shifts + `try_from`.)*
- [ ] `RawBinary` encoder; CLI `export` + `validate-export`; MCP export tools. _Done except MCP:_ `encode_raw_binary` + `row_scan_config`/`column_scan_config` presets ([#54](https://github.com/enthal/glyph-80/pull/54)); CLI `add-export-config`, `validate-export`, and `export` (validates then writes the raw `.bin`; CLI-vs-library byte-exact + dry-run + non-1:1-rejection tests) ([#55](https://github.com/enthal/glyph-80/pull/55)). MCP export tools are later.
- [ ] GUI: export-config editor, export inspector, export preview (logical image / raw-bytes view). _In progress:_ **object creators** — an Insert menu adds a glyph set (dialog) or a row-scan export config, each an undoable op, on top of the new `add_glyph_set`/`add_export_config`/`replace_export_config` domain ops ([#56](https://github.com/enthal/glyph-80/pull/56), [#57](https://github.com/enthal/glyph-80/pull/57)); the **Export Configuration editor pane** — a form (name / source glyph set / scan / code-bits) that rebuilds the config via a scan preset on Apply (id-preserving, one undo entry) with a live 1:1 validation summary, plus click-to-select in the document browser ([#58](https://github.com/enthal/glyph-80/pull/58)). Still to come: page-subset editing, an export **preview** (logical image / raw-bytes view), and a fuller inspector.
