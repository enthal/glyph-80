...
- [ ] Clipboard integration (custom fragment MIME + `text/plain` + `image/png`); cross-instance paste. _In progress:_ glyph editor `Cmd/Ctrl+C`/`V` copy/paste via the system clipboard, carrying canonical fragment JSON as `text/plain` (already cross-instance + CLI-compatible) ([#37](https://github.com/enthal/glyph-80/pull/37)). Still to come: the dedicated fragment MIME, human-readable `text/plain`, `image/png` preview, and copy/paste from the multi-glyph views.
