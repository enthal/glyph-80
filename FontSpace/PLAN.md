...
- [ ] `RawBinary` encoder; CLI `export` + `validate-export`; MCP export tools. _In progress:_ `encode_raw_binary` + a `row_scan_config` preset builder landed with the export crate ([#54](https://github.com/enthal/glyph-80/pull/54)); the CLI commands are the next slice. MCP tools are later.
