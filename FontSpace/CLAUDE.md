...
- **Map naming:** `things_by_key` for a `Map<key, thing>` **field**; "things" plural; a stored `Map<code, glyph>` is `glyphs_by_code` (not `glyph_by_code`). For nested: `things_by_inner_by_outer` means `Map<outer, Map<inner, thing>>` (read right-to-left). For collection values include the container: `glyph_vecs_by_page`. A **singular accessor** that returns one item (`fn …(&self, key) -> Option<&Thing>`) is named `thing_by_key`, not `things_by_key` — e.g. `GlyphPage::glyph_by_code`, matching spec/03 §3.6. The pluralization tracks what is returned: a map of many → plural; an accessor of one → singular.
