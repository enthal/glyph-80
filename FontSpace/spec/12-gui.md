...
**Rectangular pixel-region selection.** Holding **Shift** while dragging defines a rectangular marquee over glyph cells instead of painting (the gesture's kind is fixed at press). The marquee is drawn as an outline over the matrix and persists until re-selected, cleared, or the selection navigates to another glyph (it is tied to that glyph's grid). While a selection exists, region operations apply to it as **one** `SetPixels` each — one undo entry: **flip** (reverse) mirrors the region left↔right or top↔bottom in place; **copy** captures the region's pixels to a region clipboard and **paste** stamps that patch with its top-left at the marquee's top-left, *replacing* those cells (clipped at the glyph edge) — the region clipboard survives navigation, so you can copy a region on one glyph and paste it onto another; and **rotate** turns the region 90° clockwise, in place for a square selection and into its transposed footprint (dims swapped, anchored top-left, clipped at the glyph edge) for a non-square one, with the marquee following it there. Region controls appear only while a selection exists, so the editor is unchanged until you Shift+drag.
...
**Drag-select and multi-glyph copy.** Dragging across thumbnails selects an inclusive **run of codes** in display order (a cool-tinted highlight, set apart from the warm active-code outline), anchored where the drag began and resolved against the ordered entries so a stale anchor never yields a bogus range. While a run is selected, a bar appears with the count and **Copy**, **Blank**, **Invert**, **Shift** (Left/Right/Up/Down + Wrap), and **Clear** buttons — copy (the Copy button or `Cmd/Ctrl+C`) writes the whole run to the clipboard as one canonical multi-glyph fragment (spec/08 §8.5), undrawn codes in the run included as blank glyphs so the block's layout is preserved; **Blank**, **Invert**, and **Shift** (Left/Right/Up/Down, with the same **Wrap** toggle the editor's glyph shift uses — spec/12 §12.3) apply the corresponding batch transform (chapter 7 §7.5) to every stored glyph in the run as one undo entry each (a dangling code — stored but not in the character set — is skipped, since the transforms are charset-resolved); Clear drops the run. There are two ways to paste back. **`Cmd/Ctrl+V`** drops the OS-clipboard fragment onto the current page starting at the selected code (`SequentialFromCode`) — this reads the system clipboard, so it works across windows and with the CLI. **Paste (by code)** — a button in the page-overview header, shown once you have copied glyphs this session — restamps the last copied run so each glyph lands on **its own original code** (`ByCode`), from an in-app fragment clipboard (distinct from the OS clipboard, same session only); a code with no entry on the target page is reported rather than created. Both paste as one undo entry. A single click (or navigating away) supersedes the run. The run selection is UI state, tied to the page it was drawn on; it is never persisted.
...
**Interim: object creators live under an `Insert` menu.** The full taxonomy above and the single command-tree/two-presenter machinery are a later (Milestone-4) slice. Until then, the object-creation actions that exist land under one **`Insert`** menu in the current `egui` bar: **Glyph Set…** (name + geometry + character-set picker → `AddGlyphSet`, spec/07 §7.2) and **Export Configuration** (a row-scan config from the selected glyph set → `AddExportConfig`). When the command-tree menu system arrives these move to their taxonomy homes (glyph-set creation under a Glyph/Glyph-Set menu, export creation under **Export**); the `Insert` grouping is a scaffold, not the target design.
