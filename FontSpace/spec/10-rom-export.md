...
**Output size and fill.** A config carries an optional `output_size` (in **bytes**) and a `fill_byte` (default `0xFF`, the erased-EEPROM value). When `output_size` is set it must be a **power of two** no smaller than the natural image; the encoder pads the image up to it with `fill_byte`, so a small ROM fills a larger EEPROM (minipro and friends). `None` means the natural size. `render_rom` is the one entry point that validates, generates, encodes, and pads — it never emits bytes from a config that isn't a valid strict-1:1 mapping. (Once the memory-mapped multi-bank model lands, `fill_byte` also fills any address not covered by a bank.)
