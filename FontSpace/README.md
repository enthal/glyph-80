...
`extract` copies the selected glyphs off one page (`--page` must match exactly one; `--glyphs` defaults to all) into a canonical **fragment** file (`application/x-fontspace-fragment+json`, spec/08 §8.5) for reuse elsewhere. Mutating commands (`set-pixels`, `shift`) write canonical JSON atomically and support `--dry-run` (which `extract` also honors, writing nothing); `--seq` wires the deterministic id generator for reproducible fixtures. (Fragment `paste`, multi-page/whole-page fragments, and machine-readable JSON errors arrive with later milestones.)
