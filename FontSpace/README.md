...
`extract` copies the selected glyphs off one page (`--page` must match exactly one; `--glyphs` defaults to all) into a canonical **fragment** file (`application/x-fontspace-fragment+json`, spec/08 §8.5); `paste` places a fragment back onto one page under an explicit `--mapping` (`by-code` or `sequential-from-code:CODE`) and `--size` policy (`require-exact` — no silent resize — `center`, or `place-at:X,Y`, the placements clipping rather than resampling). Mutating commands (`set-pixels`, `shift`, `paste`) write canonical JSON atomically and support `--dry-run` (which `extract` also honors, writing nothing); `--seq` wires the deterministic id generator for reproducible fixtures. (Multi-page/whole-page fragments, `by-slot`/`crop`/`scale-nearest` paste, and machine-readable JSON errors arrive with later milestones.)
